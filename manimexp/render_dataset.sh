...
#   ./manimexp/render_dataset.sh <datasetId> [N] [seed]
...
export PYTHONPATH=.
MANIM=manimexp/.venv/bin/manim
DRAFTS=assets/manim/isomap/drafts
mkdir -p manimexp/isomap/points "$DRAFTS"

node manimexp/isomap/gen_points.mjs "$DS" "$N" "$SEED" "manimexp/isomap/points/${DS}_${N}.json"

MFI_DATASET="$DS" MFI_N="$N" "$MANIM" -ql --fps 30 --disable_caching \
  manimexp/isomap/walkthrough.py IsomapWalkthrough
